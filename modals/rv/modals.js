...
export function DeleteTransaction({ mount, transaction, reload, unmount }) {
	...
	async function doDeletion() {
		try {
			...
			const body = { payload: transaction.transaction, type: "delete-rv-transaction" };
			const response = await axios.post(ApiEndpoints.Setter, body, MyGlobal.GetHeaders());

			if (response.status === 200) {
				reload();

				MyGlobal.AddActivity(`Deleted transaction <b>${transaction.transaction.particulars}</b> having amount <b>${transaction.transaction.amount}</b> of <b>${transaction.transaction.project_id}</b>`, BaseModules.Rv);
				MyGlobal.ShowSuccessToast("RV transaction deleted.");
			...
			MyGlobal.HandleErrors(error, "Delete RV Transaction");
		} finally {
	...
	function uiTitleBar() {
		return (
			<DialogTitle as="h2" className={titleBarStyle}>
				<span className="flex w-full justify-start items-center">Delete RV Transaction</span>
...
export function EditAmount({ invoice, mount, reload, unmount }) {
	...
	async function doEditing() {
		...
		const body = {
			...
			type: "edit-rv-transaction-amount",
		...
		try {
			...
			if (response.status === 200) {
				...
				MyGlobal.AddActivity(`Edited transaction amount of <b>${invoice.transaction.project_id}</b> <b>${invoice.company_name}</b> (RV ID - <b>${invoiceCustomId}</b>) from <b>${invoice.transaction.amount}</b> to <b>${main.amount}</b>.`, BaseModules.Rv);
			...
			MyGlobal.HandleErrors(error, "Edit RV Transaction Amount");
		} finally {
