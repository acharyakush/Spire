...
export default function ProjectTodos({ mount, project, reload, unmount }) {
  ...
  async function addNote() {
    try {
      ...
      const suffix =
        ...
        MyGlobal.GetUserId() +
        "::" +
        dayjs().format("hh:mm a, DD MMM, YYYY");

      let notesTimeline = main.activeModuleData.notes
        ? [main.newNote + suffix, main.activeModuleData.notes + suffix]
        : [main.newNote + suffix];
      ...
      const response = await axios.post(
        MyConstants.ApiEndpoints.Todos.EditDetails,
        body,
        MyGlobal.GetHeaders()
      );
  ...
  async function updateStatus(status) {
    try {
      ...
      const response = await axios.post(
        MyConstants.ApiEndpoints.Setter,
        body,
        MyGlobal.GetHeaders()
      );
  ...
  function uiAssignedTo() {
    const assignedToNames = MyGlobal.GetAnyDataFromId(
      main.activeModuleData.assigned_to,
      "full_name"
    );
        ...
        <div className="flex w-full justify-between items-center">
          ...
          {main.activeModuleData.due_date && (
            <div className="flex flex-col w-1/2 justify-center items-end">
              ...
              <span className="font-medium-12 text-red-500">
                {dayjs(main.activeModuleData.due_date).format("DD MMM, YYYY")}
              </span>
            </div>
          )}
        </div>
  ...
  function uiDescription() {
    const description = main.activeModuleData.description_timeline
      ? JSON.parse(main.activeModuleData.description_timeline)
      : "";
  ...
  function uiNotes() {
    try {
      const notesTimeline = main.activeModuleData.notes_timeline
        ? safeJsonParse(main.activeModuleData.notes_timeline)
        : [];

      return (
        <div className="flex flex-col w-full justify-start items-center ">
          <span className="w-full text-left font-regular-10 gray-text">
            Notes
          </span>
          <div className="w-full h-[200px] divide-y overflow-y-auto font-regular-12 space-y-2 p-4 bg-white rounded full-border">
            {notesTimeline
              ? notesTimeline?.map((m, i) => {
                  ...
                  const userFullName = MyGlobal.GetAnyDataFromId(
                    userId,
                    "full_name"
                  );
    ...
    return (
      <div className="flex flex-col w-full h-[77px] justify-center items-start">
        ...
        <span className="font-medium-12 black-text">
          {main.activeModuleData.priority}
        </span>
      </div>
    );
  }
    ...
    return (
      <DialogTitle as="h2" className={titleBarStyle}>
        <span className="flex w-full justify-start items-center">
          {project.client_name} - {project.sub_project_name} (
          {project.main_project_name}) :: Todos
        </span>
        <FontAwesomeIcon
          ...
          <DialogPanel className="w-4/5 transform overflow-hidden rounded shadow contrast-background">
            ...
            <div className="flex flex-col w-full py-6 space-y-3 justify-between items-center">
              <div className="flex w-full px-6 justify-center items-start space-x-5">
                <div className="flex flex-col w-[10%] space-y-3 justify-start items-center">
                  {uiList()}
                </div>
                <div className="flex flex-col w-[90%] h-full justify-start items-center full-border rounded bg-gray-50">
                  {uiBody()}
                </div>
              </div>
              <div className="flex w-full px-6 justify-center items-end space-x-5">
                ...
                <div className="flex w-[90%] space-x-5 justify-center items-center">
                  <TextArea
                    ...
                    label="Add Note"
                    onChange={(e) =>
                      setMain((s) => ({ ...s, newNote: e.target.value }))
                    }
                    tabIndex={1}
                    width="w-full"
                    value={main.newNote}
