...
export default function EditTransaction({ mount, reload, transaction, unmount }) {
	...
	const pendingFees = MyGlobal.GetNumbers(transaction.project.pending_fees);
	...
	async function doEditing() {
		...
		const body = {
			affiliateId: transaction.affiliate_id,
	...
	function uiPaymentType() {
		return <ComboBox allowCreatingNewItem={false} comparisonValue="" filteredData={api.paymentTypes} icon={faFile} label="Payment Type" onChange={(e) => setInputs("paymentType", e)} onClick={() => {}} onKeyPress={() => {}} searchedItem="" tabIndex="6" value={main.paymentType} width="w-full" />;
	}
