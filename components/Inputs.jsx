...
export const ComboBox = ({ allowCreatingNewItem, comparisonValue, filteredData, icon, isReadOnly = false, label, onChange, onClick, onKeyPress, searchedItem, tabIndex, value, width }) => {
				...
				<div className={childWrapper}>
					<Combobox onChange={onChange} onKeyPress={onKeyPress} value={value}>
						<div className="relative w-full">
							...
							<ComboboxOptions className="absolute w-full max-h-37.5 mt-1 overflow-auto divide-y rounded bottom-shadow outline-none z-50 full-border primary-light-background">{uiList()}</ComboboxOptions>
						</div>
					</Combobox>
				</div>
			</div>
	...
	const uiList = () => {
		if (!_filteredData) {
			...
			return (
				<div className="flex w-full p-2 justify-between items-center cursor-pointer font-regular-11 black-text" onClick={noItemFoundClickAction}>
					{noItemFound}
				</div>
			);
		} else {
...
export const ComboBox2 = ({ allowCreatingNewItem, comparingValue1, comparingValue2, displayValue, filteredData, hasDataObject, icon, isMenuInverted = false, isMultipleDisplayValue = false, isReadOnly, label, multipleDisplayValue = [], onChange, onClick, onInputChange, onKeyPress, searchedItem, showFullObject = false, tabIndex, value, width }) => {
				...
				<div className="flex w-full justify-start items-center">
					<Combobox onChange={onChange} onKeyPress={onKeyPress} value={value}>
						<div className={inputWrapper}>
							...
							<ComboboxOptions className="absolute w-full max-h-37.5 mt-1 overflow-auto rounded bottom-shadow outline-none z-50 full-border primary-light-background" style={{ top: isMenuInverted ? "-160px" : "36px", zIndex: 50 }}>
								{uiList()}
							</ComboboxOptions>
						</div>
					</Combobox>
...
export const ComboBoxWithChips = ({ background = "primary-background-transparent-01", displayKey, fontSize = "font-regular-11", height = "h-full", icon, iconSize = "sm", isMenuInverted, label, onBlur, onItemClick, onSelectedItemClick, padding = "p-2", selectedItems, showInitials = false, showList, source, subIcon = faUser, toggleMenu, topPosition = "-90px", width = "w-full" }) => {
	const uiBox = () => {
		...
		return (
			<div className={wrapper} onBlur={onBlur}>
				...
				<div className={childWrapper}>
					...
					<div className="flex w-full h-8.5 pl-2.5 justify-between items-center relative">
						<div
				...
				<div className={showList} style={{ top: isMenuInverted ? topPosition : "66px", zIndex: 50 }}>
					{uiList()}
				</div>
			</div>
		);
			...
			return (
				<span className={wrapper} key={n} onClick={() => onItemClick(m)}>
					<div className="flex pr-2 space-x-2 justify-center items-center">
						<FontAwesomeIcon className="primary-text" icon={subIcon} />
...
export const DatePicker = ({ background = "primary-background-transparent-01", fontSize = "font-regular-11", height = "h-9", icon, label, onChange, showLabel = true, placeholder = "", iconSize = "", padding = "p-2", gap = "space-x-2", tabIndex, value, width }) => {
	...
	return (
		<div className={mainWrapper}>
			...
			<div className={wrapper}>
				<FontAwesomeIcon className="primary-text" icon={icon} size={iconSize} />
				<ReactDatePicker autoFocus={false} className={datePickerStyle} dateFormat="dd-MM-yyyy" dropdownMode="select" onChange={onChange} placeholderText={placeholder} peekNextMonth selected={value} showMonthDropdown showYearDropdown tabIndex={tabIndex} />
			</div>
		</div>
	);
};
