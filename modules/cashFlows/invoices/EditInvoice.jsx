...
export default function EditInvoice({ project, reload, unmount }) {
	...
	function uiInputFinancialYear() {
		return (
			<TextInput
				...
				maxLength={7}
				onChange={(e) => {
					setMain((s) => ({ ...s, invoiceId: MyGlobal.MakeNewInvoiceId(main.firm.name, api.invoices, DerivedModules.EditInvoice, main.firm, e.target.value) }));
	...
	function uiInputInvoiceDate() {
		return (
			<DatePicker
				...
				label="Invoice Date"
				onChange={(e) => {
					setMain((s) => ({ ...s, invoiceId: MyGlobal.MakeNewInvoiceId(main.firm.name, api.invoices, DerivedModules.EditInvoice, main.firm, getFinancialYearByDate(e)) }));
	...
	function uiInputInvoiceId() {
		return <TextInput icon={faHashtag} isReadOnly label="ID" maxLength={5} onChange={(e) => setInputs("invoiceId", e.target.value)} onKeyPress={(e) => !MyGlobal.HasNumbers(e.key) && e.preventDefault()} tabIndex={2} value={main.invoiceId} width="w-full" />;
	}
	...
	function uiInputAmount(object) {
		return <TextInput icon={faIndianRupee} id={`amount${object.rowId}`} label="Amount" onChange={(e) => setParticulars("amount", object.rowId, e.target.value)} onKeyPress={(e) => !MyGlobal.HasNumbers(e.key) && e.preventDefault()} tabIndex={`${object.rowId}2`} value={object.amount} width="w-full" />;
	}
