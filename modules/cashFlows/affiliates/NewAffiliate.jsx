...
import { faChevronRight, faCircleMinus, faCode, faFont, faHashtag, faPhone, faPlusCircle, faTriangleExclamation, faUser } from "@fortawesome/free-solid-svg-icons";
...
export default function NewAffiliate({ reload, unmount }) {
	...
	async function addToDatabase() {
		...
		try {
			...
			const response = await axios.post(ApiEndpoints.Affiliates.AddAffiliate, body, MyGlobal.GetHeaders());

			if (response.status === 200) {
				reload();

				MyGlobal.AddActivity("Added affiliate(s).", BaseModules.Affiliates);
	...
	async function setAllAffiliates() {
		...
		try {
			const response = await axios.get(ApiEndpoints.Getter, MyGlobal.GetHeaders({ type: "get-affiliates" }));
	...
	function uiBankAccountHolderName(object, rowId) {
		return <TextInput icon={faFont} id={`bankAccountHolderName${rowId}`} label="Bank Account Holder Name" onChange={(e) => setInputs("bankAccountHolderName", rowId, e.target.value)} onKeyPress={() => {}} tabIndex={`${rowId}4`} value={object.bankAccountHolderName} width="w-full" />;
	}

	function uiBankAccountNumber(object, rowId) {
		return <TextInput icon={faHashtag} id={`bankAccountNumber${rowId}`} label="Bank Account Number" onChange={(e) => setInputs("bankAccountNumber", rowId, e.target.value)} onKeyPress={() => {}} tabIndex={`${rowId}5`} value={object.bankAccountNumber} width="w-full" />;
	}

	function uiEmailAddress(object, rowId) {
		return <EmailAddress label="Email Address" onChange={(e) => setInputs("emailAddress", rowId, e.target.value)} suffix="" tabIndex={`${rowId}3`} value={object.emailAddress} width="w-full" />;
	}

	function uiEmailAddress1(object, rowId) {
		return <EmailAddress label="Email Address" onChange={(e) => setInputs("emailAddress", rowId, e.target.value)} suffix="" tabIndex={`${rowId}3`} value={object.emailAddress} width="w-full invisible" />;
	}

	function uiIfsc(object, rowId) {
		return <TextInput icon={faCode} id={`ifsc${rowId}`} label="IFS Code" onChange={(e) => setInputs("ifsc", rowId, e.target.value)} onKeyPress={() => {}} tabIndex={`${rowId}6`} value={object.ifsc} width="w-full" />;
	}

	function uiName(object, rowId) {
		return <TextInput icon={faUser} id={`name${rowId}`} label="Name" onChange={(e) => setInputs("name", rowId, e.target.value)} onKeyPress={() => {}} tabIndex={`${rowId}1`} value={object.name} width="w-full" />;
	}

	function uiPhoneNumber(object, rowId) {
		return <TextInput icon={faPhone} id={`phoneNumber${rowId}`} label="Phone Number" onChange={(e) => setInputs("phoneNumber", rowId, e.target.value)} onKeyPress={(e) => !MyGlobal.HasNumbers(e.key) && e.preventDefault()} tabIndex={`${rowId}2`} value={object.phoneNumber} width="w-full" />;
	}

	function uiUpiId(object, rowId) {
		return <TextInput icon={faGooglePay} iconSize="2x" id={`upiId${rowId}`} label="Unified Payment Interface ID" onChange={(e) => setInputs("upiId", rowId, e.target.value)} onKeyPress={() => {}} tabIndex={`${rowId}7`} value={object.upiId} width="w-full" />;
	}
					...
					<div className="flex w-full space-x-2.5 justify-start items-center">
						<div className="flex w-full space-x-2.5 justify-start items-center">
							<span className="cursor-pointer view-heading" onClick={() => unmount()}>
								{BaseModules.Affiliates}
							</span>
							<FontAwesomeIcon className="gray-text" icon={faChevronRight} size="xs" />
							<span className="view-heading">{DerivedModules.NewAffiliate}</span>
						</div>
					</div>
				</div>
