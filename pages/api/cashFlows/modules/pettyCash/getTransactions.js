...
export default async function handler(req, res) {
	...
	try {
		const [ownerFirms, ownerFirmsBanks, settings, openingBalance, transactions] = await Promise.all([
		...
		return res.status(200).json({ ownerFirms, ownerFirmsBanks, settings, openingBalance, transactions });
	} catch (error) {
		console.error(error);
