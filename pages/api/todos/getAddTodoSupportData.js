...
export default async function handler(req, res) {
	...
	try {
		const [clients, projects, mainProjects, subProjects, companies] = await Promise.all([query("SELECT * FROM clients WHERE is_confirmed=1 AND is_deleted=0 ORDER BY id DESC", []), query("SELECT * FROM projects WHERE is_deleted=0 AND status='Active' ORDER BY id DESC", []), query("SELECT * FROM main_projects", []), query("SELECT * FROM sub_projects", []), query("SELECT * FROM companies WHERE is_deleted = 0", [])]);
