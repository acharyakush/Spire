...
export default async function handler(req, res) {
	...
	try {
		...
		for (let object of group) {
			...
			const result = await query(`INSERT INTO affiliates (id, name, email_address, phone_number, bank_account_holder_name, bank_account_number, ifsc, upi_id, entry_by_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [response.new_id, name, emailAddress ?? "", phoneNumber ?? "", bankAccountHolderName ?? "", bankAccountNumber ?? "", ifsc ?? "", upiId ?? "", userId]);
