...
export default async function handler(req, res) {
	...
	try {
		const [affiliates, clients, companies, mainProjects, subProjects] = await Promise.all([
		...
		return res.status(200).json({ affiliates, clients, companies, mainProjects, subProjects });
	} catch (error) {
		console.error(error);
