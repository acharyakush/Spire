...
export default async function handler(req, res) {
	...
	try {
		...
		const insertQueryResult = await query(
			"UPDATE affiliates_transactions SET firm_id=?, bank_id=?, amount=?, particulars=?, payment_source=?, payment_type=?, remarks=?, entry_at=? WHERE id=? AND affiliate_id=? AND project_id=?",
			[firmId, bankId, amount, particulars, paymentSource, paymentType, remarks, entryAt, id, affiliateId, projectId],
		);
