...
export default async function handler(req, res) {
	...
	try {
		const [cashFlows, clients, invoices, ownerFirms, ownerFirmsBanks] = await Promise.all([
		...
		return res.status(200).json({ cashFlows, clients, invoices, ownerFirms, ownerFirmsBanks });
	} catch (error) {
		console.error(error);
