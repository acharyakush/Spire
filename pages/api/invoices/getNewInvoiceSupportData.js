...
export default async function handler(req, res) {
	...
	try {
		const [cashFlows, clients, companies, invoices, ownerFirms, ownerFirmsBanks] = await Promise.all([
		...
		return res.status(200).json({ cashFlows, clients, companies, invoices, ownerFirms, ownerFirmsBanks });
	} catch (error) {
		console.error(error);
