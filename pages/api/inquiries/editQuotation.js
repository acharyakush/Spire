...
export default async function handler(req, res) {
	...
	try {
		...
		await query("UPDATE inquiries_quotations SET firm_id = ?, client_id = ?, client_address = ?, date = ?, remarks = ?, terms_conditions = ? WHERE custom_id = ?", [
			firmId,
			clientId,
			clientAddress,
			date,
		...
		await Promise.all(
			services.map((s) =>
				query("UPDATE inquiries_quotations_services SET services = ?, inclusions = ?, professional_fees = ?, government_fees = ? WHERE quotation_id = ?", [
					s?.services,
					s?.inclusions,
					s?.professionalFees,
					s?.governmentFees,
