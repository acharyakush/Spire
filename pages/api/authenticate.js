...
import { decrypt, encrypt } from "@/utilities/myGlobal";
...
export default async function handler(req, res) {
	if (!MyGlobal.IsApiCallMethodValid(req)) {
		...
		try {
			...
			const decryptedBody = decrypt(req.body.credentials);
			...
			const response = await query(`SELECT * FROM ${tableName} WHERE email_address=? AND password=?`, [emailAddress, encrypt(body.password)]);
			...
			if (!response.length) {
				...
				const encryptedResponse = encrypt(jsonResponse);
