...
export default async function handler(req, res) {
	...
	try {
		const [administratorsCompanies, clients, companies, inquiries, mainProjects, subProjects] = await Promise.all([
		...
		return res.status(200).json({ administratorsCompanies, clients, companies, inquiries, mainProjects, subProjects });
	} catch (error) {
		console.error(error);
