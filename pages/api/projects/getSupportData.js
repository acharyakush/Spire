...
export default async function handler(req, res) {
	...
	try {
		const [clients, companies, inquiries, mainProjects, ownerFirms, subProjects] = await Promise.all([
		...
		return res.status(200).json({ clients, companies, inquiries, mainProjects, ownerFirms, subProjects });
	} catch (error) {
		console.error(error);
