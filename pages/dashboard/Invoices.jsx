...
export default function Invoices({ invoices, setModuleProps }) {
	...
	function uiBlock(key) {
		...
		const amount = key == "NOT GENERATED" ? invoices?.notGenerated?.amount : invoices?.[_key]?.amount;
		const count = key == "NOT GENERATED" ? invoices?.notGenerated?.count : invoices?.[_key]?.count;
